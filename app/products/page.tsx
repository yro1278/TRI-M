...
import { Search, X, Plus, Edit2, Trash2, Grid3X3, List, AlertCircle, RefreshCw, Package } from "lucide-react";
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-4">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="bg-surface rounded-xl border border-border/50 overflow-hidden">
    ...
    <motion.div variants={container} initial="hidden" animate="visible" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <motion.div variants={fadeUp} className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-muted/60 mt-1">{filtered.length} of {products.length} products</p>
        </div>
        <button onClick={() => { setEditing(null); setShowForm(true); }} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-primary-dark transition-all">
      ...
      <motion.div variants={fadeUp} className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between mb-6">
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`shrink-0 rounded-lg px-2.5 py-1 text-sm font-medium transition-all ${
                activeCategory === cat ? "bg-primary text-white shadow-sm" : "text-muted/70 hover:text-foreground hover:bg-border/30"
              }`}>
        ...
        <div className="flex items-center gap-2">
          <div className="relative group">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..." className="w-48 rounded-lg border border-border/40 bg-surface/50 py-1.5 pl-8 pr-7 text-sm text-foreground outline-none transition-all focus:border-primary/30 focus:bg-surface" />
          ...
          <div className="rounded-lg border border-border/30 overflow-hidden">
            <button onClick={() => setViewMode("grid")} className={`p-1.5 transition-colors ${viewMode === "grid" ? "bg-primary text-white" : "text-muted/60 hover:bg-border/30"}`}><Grid3X3 className="h-3.5 w-3.5" /></button>
            <button onClick={() => setViewMode("list")} className={`border-l border-border/30 p-1.5 transition-colors ${viewMode === "list" ? "bg-primary text-white" : "text-muted/60 hover:bg-border/30"}`}><List className="h-3.5 w-3.5" /></button>
          </div>
        </div>
      </motion.div>
        ...
        {filtered.length === 0 ? (
            ...
            {filtered.map((product) => (
              <div key={product.id} className="relative group">
                ...
                <div className="absolute right-2 top-2 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditing(product); setShowForm(true); }} className="bg-background/80 backdrop-blur-sm rounded-lg p-1.5 text-muted/70 hover:text-primary transition-colors"><Edit2 className="h-3 w-3" /></button>
                  <button onClick={() => handleDelete(product.id)} className="bg-background/80 backdrop-blur-sm rounded-lg p-1.5 text-muted/70 hover:text-red-500 transition-colors"><Trash2 className="h-3 w-3" /></button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-surface rounded-xl border border-border/50 overflow-hidden">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {filtered.map((p) => (
                  <tr key={p.id} className="border-b border-border/10 last:border-0 hover:bg-border/20 transition-colors">
                    <td className="p-3">
                      <div className="flex items-center gap-2.5">
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-primary/[0.08] to-primary/[0.02] text-xs font-medium text-primary ring-1 ring-primary/10">{p.image}</div>
                        <div><p className="text-sm text-foreground">{p.name}</p><p className="text-xs text-muted/60">{p.brand}</p></div>
                      </div>
                    </td>
                    <td className="p-3 text-muted/70">{p.sku}</td>
                    ...
                    <td className="p-3 text-muted/70">${p.cost.toFixed(2)}</td>
