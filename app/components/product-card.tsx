...
export default function ProductCard({ product }: { product: Product }) {
  return (
    <div className="card-hover bg-card rounded-xl shadow-sm border border-border/20 overflow-hidden">
      <div className="relative aspect-square bg-gradient-to-br from-border/20 to-border/5 flex items-center justify-center overflow-hidden">
        ...
        <span className="text-5xl opacity-15 select-none">{product.image}</span>
      </div>
      <div className="p-4 space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-semibold uppercase tracking-wider text-primary/70">{product.brand}</span>
          <span className="text-[10px] text-muted/25">{product.sku}</span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-muted/30">({product.reviews})</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-base font-bold text-foreground">${product.price.toFixed(2)}</span>
          {product.originalPrice && (
            <span className="text-xs text-muted/30 line-through">${product.originalPrice.toFixed(2)}</span>
          )}
        </div>
